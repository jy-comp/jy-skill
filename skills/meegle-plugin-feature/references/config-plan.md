...
> **必填字段 / 长度 / 枚举合法性以 `npx @byted-meego/cli@builder schema` 输出为准**，由 `local-config set` 强制校验，本表不再镜像——避免 schema 迭代后漂移。schema 抓不到的行为指引（URL 询问、MCP 强制查询、派生字段规则等）见本文档（`references/config-plan.md`）的"各类型的行为指引"节。
...
用户原话的读取方式：
- **被 meegle-plugin-workflow 编排时**：从 `.lpm-cache/state.json` 的 `context.originalRequirement` 读（Phase 0 逐字记录的版本，不受会话压缩影响）
- **独立调用时**：从当前对话上下文读
...
完整判定规则（含 Runtime / Metadata 分类表与所有 URL 字段清单）见 [`meegle-plugin-shared/references/url-policy.md`](../../meegle-plugin-shared/references/url-policy.md)。本节补充 P3 阶段的具体执行动作。
...
- **(B) 先占位** → 写入 `"<PLACEHOLDER: 请替换为你的<字段语义>>"`；若 schema 拒绝纯文本占位符，按 `meegle-plugin-shared` 的"规则冲突兜底"向用户上报再决策；P6 摘要用 🔴 醒目标注，meegle-plugin-publish 的 pre-check 会硬阻止
...
> set 是全量替换（规则定义见 [`../../meegle-plugin-shared/SKILL.md`](../../meegle-plugin-shared/SKILL.md) 的"全量提交约束"）——基于 `.lpm-cache/config/remote.json` 的远端全量配置操作，而不是只传变更部分。
...
### 删除操作（CRITICAL · 根原则 2 数据完整性 · config.plan 实施点）
