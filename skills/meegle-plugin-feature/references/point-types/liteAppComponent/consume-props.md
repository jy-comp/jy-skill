...
组件需要一个或多个 property，每项有 `propKey` + `propType`（声明归 Stage Config，声明路径见 [`../../config-plan.md`](../../config-plan.md) P0）。**propType 只能从下表枚举取**（源：MCP `LiteAppPropTemplateType` 枚举）：
...
**watch 的作用域**：`watch` 订阅**所有 propKey 的变化**，包括 dataSource 类型。回调入参 `newProps[propKey]` 在不同 propType 下含义不同：
- 普通属性（text / number / select / viewSelect / ...）：拿到的是**配置值本身**（管理员当前设置的值，与 `getProps()` 同形态）
- dataSource 属性：拿到的是**工作项数据**（响应式，上游推送/筛选/绑定切换时自动更新），不是订阅配置元信息

要拿 dataSource 的元信息（指向哪个上游）只能通过 `getProps()`，且是静态的；要响应式拿数据走 watch；详见 [`consume-data.md`](consume-data.md) 步骤 5。

**`watchKeys` 参数形态**：支持两种写法
- `watch(cb, 'titleKey')` — 单 propKey string（订阅一个）
- `watch(cb, ['titleKey', 'layout'])` — propKey 数组（订阅多个）
- 不传则监听全部可监听 key
...
- **dataSource 属性的三套 API 不要混**：
  - `getProps()` → 静态**元信息**（指向哪个上游），不是工作项数据
  - `watch(cb, dataSourceKey)` → 响应式**数据**（回调里 `newProps[dataSourceKey]` 直接是工作项列表）
  - `getDataSourceResult(propKey, {pageNum, ...})` → 主动拉**数据**（mount 初始化 / 翻页）
  - 详见 `consume-data.md` 步骤 5
...
> 注：propType 枚举、option 结构、声明字段格式等"声明端的 schema"问题不归 Stage Code —— 见 [`../../code-plan.md`](../../code-plan.md) P0 边界，那是本 skill 的 Stage Config 的事。
