...
（`plugin.config.json` 的维护规则见 [`../../meego-shared/SKILL.md`](../../meego-shared/SKILL.md#安全规则)。）

## A1.5：代码溯源审计（脚本化，跨平台）

A1 写完后、A2 输出前，**MUST 执行下述 bash 审计脚本**对所有改动文件做方法名存在性校验，避免 AI 在写代码时悄悄从 schema 字段名脑补 SDK 调用。

> 本步骤用纯 Bash + Grep 实现，不依赖 subagent / Agent 工具，Claude Code 与 Codex CLI 均可执行。
...
这些语义层问题靠 plan.md 的"代码溯源协议 + 无源即停"在前置阶段堵住。脚本是**事后兜底闸**，不是全部防线。
