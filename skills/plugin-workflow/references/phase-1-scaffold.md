...
**Checkpoint 写入**：`{ "phase": 1, "step": "1.1", "stepName": "env-setup", "nextCommand": "npx @byted-meego/cli@builder login ...", "nextStep": "1.1 环境准备" }`

```
调用 /env-setup
```

**判断逻辑（两步预检，plugin-workflow 作为 orchestrator 专享）：**
...
2. **Auth probe**（使用 `context.siteDomain`）：
   ...
   ```
   - `exit 0` → 已登录 → **跳过 env-setup**，直接进入 1.2 plugin-create
   - `exit 1` → stderr 已含完整登录指引 → **触发 `/env-setup`** 执行登录编排（方式 A token 粘贴 / 方式 B Device Code 后台驱动），完成后继续 1.2

> **为什么 plugin-workflow 做预检而其它 skill 不做**：plugin-workflow 是多步编排的 orchestrator，中断在链条中间（如 plugin-create 报 auth 错）体验差。其它 skill（meego-point-config、plugin-code-gen 等）独立调用时规则简单即可，依赖 CLI 内置的 preflight 兜底，不需要每个都加一次 probe。
>
> **为什么用 probe 而不是读 auth.json**：skill 不直接耦合 CLI 的 token 存储格式。probe 命令是 CLI 对外暴露的稳定接口，auth.json 的内部结构可能演进。

## 1.2 创建插件 → plugin-create

从 `context.originalRequirement` 派生工作名称，最小化创建：

**Checkpoint 写入**：`{ "phase": 1, "step": "1.2", "stepName": "plugin-create", "nextCommand": "npx @byted-meego/cli@builder create ...", "nextStep": "1.2 创建插件" }`
...
**plugin-create 成功后 MUST 切 cwd 到新工程目录**，再继续 Phase 2（后续 CLI 命令、Read `plugin.config.json` 等都依赖 cwd 指向新工程）。
