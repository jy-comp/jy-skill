...
**用户已指定点位的情况：** 如果用户在 Phase 1 中已经明确指定了点位类型（如"我需要一个轻应用组件"），则：
- **优先使用用户指定的点位**，在 schema 中验证该点位确实存在
- AI 的额外推荐仅作为补充建议，优先级低于用户选择
- 不要用 AI 推荐覆盖用户的明确指定

### 2.3.3 向用户确认点位方案

拉取 schema 并分析后，向用户展示点位方案。

**用户已指定点位时：**

```
已在 schema 中确认你指定的点位类型可用：

🧩 点位：
   - [用户指定的点位类型]：[schema 中的原文描述]

（如有需要，也可以额外添加其他点位，如 config 用于配置页）
