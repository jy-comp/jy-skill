...
> **前置**：本段是 meego-shared 主文件"禁止编造 URL"的完整规则。涉及 URL 字段填写的 skill（meego-point-config / plugin-publish）触发时按需 Read 此文件。
...
- URL 溯源（写入前）：`meego-point-config/references/plan.md` 的 "URL 溯源协议" 章节
- URL lint（写入后）：`meego-point-config/references/apply.md` 的 A0.5 + `plugin-publish/references/pre-check.md` 的 PC1.5
